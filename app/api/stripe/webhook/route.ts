...
export async function POST(request: Request) {
  ...
  switch (event.type) {
    case 'payment_intent.succeeded':
      ...
      if (productId && productId === 'prod_T8SB2KQsiFVbHs' && user?.userId) {
        singlePayDeal(user?.userId, paymentIntent.id)
        const billingResult = await addBilling(user?.userId, paymentIntent.amount_received, paymentIntent.id)
        console.log('addBilling result:', billingResult)
      }
...
const singlePayDeal = async (userId: string, paymentIntentId: string) => {
  try {
    const key = getOrderUniqueId(paymentIntentId)
...
const memberPayDeal = async (userId: string, subscription: Stripe.Subscription) => {
  try {
    const key = getOrderUniqueId(subscription.id)
    const orderRedisRes = await redis.get(key)
    console.log('orderRedisRes', orderRedisRes);
    if (!orderRedisRes) {
