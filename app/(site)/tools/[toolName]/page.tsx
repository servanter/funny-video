...
import SingFeatures from "@/components/FeatureSing";
...
export default async function ToolPage({ params }: ToolPageProps) {
  // 根据 toolName 匹配配置
  const config = DescriptionConfig.find(
      ...
      <div className="flex max-full mx-auto flex-col justify-center py-0 px-4">
        <div className="flex flex-1 w-full flex-col items-center justify-center text-center">
          <h1 className="text-6xl font-bold mt-20">
            <span className="custom-underline relative">Funny Video</span>
          </h1>
          ...
          <SingFeatures config={config} />

          <UploadPreviewSection userUsage={userUsage} selectedImageIndex={0} />
        </div>
      </div>
