 ...
 * @returns 
 */
export const decrementBoostPack = async (userId: string, count: number = 10) => {
  const boostPackKey = await getBoostPackKey({ userId });
  try {
    const limit = await redis.get(boostPackKey); // 确保键存在 Ensure the key exists
    const remaining = await redis.decrby(boostPackKey, count);
...
export const incrMembershipUsage = async (userId: string, count: number = 10) => {
  const userMembershipUsageKey = getUserDateUsageKey({ userId });
  try {
    const current = redis.incrby(userMembershipUsageKey, count);
